...
//! through whatever template files are configured. So noctalia-iced ships one, registers it, and reads
...
const REGISTRATION: &str = "noctalia-iced.toml";
...
fn state_dir() -> Option<PathBuf> {
    match std::env::var_os("XDG_STATE_HOME") {
        Some(path) => Some(PathBuf::from(path).join("noctalia-iced")),
        None => Some(home()?.join(".local/state/noctalia-iced")),
    }
}
...
pub fn template_path() -> Option<PathBuf> {
    Some(state_dir()?.join("palette.tpl"))
}

/// Where the registration file in Noctalia config is stored.
pub fn registration_path() -> Option<PathBuf> {
    Some(noctalia_config_dir()?.join(REGISTRATION))
}
...
///
/// Both files are ours: the template under our own state directory, and a `noctalia-iced.toml` beside
...
pub fn install() -> Result<Installed, String> {
    ...
    let entry = format!(
        "# Written by `noctalia-iced`. Delete this file and\n\
         # {} to undo it.\n\
         #\n\
         # Noctalia renders this on every theme change, which is how applications using noctalia-iced\n\
         # follow the palette you are running — including built-in palettes and \"pure black\", neither\n\
         # of which can be read back out of settings.toml.\n\
         \n\
         [theme.templates.user.noctalia_iced]\n\
         input_path = \"{}\"\n\
         output_path = \"{}\"\n",
        template.display(),
        template.display(),
...
pub fn watch() -> (Option<Palette>, Changes) {
    ...
    std::thread::Builder::new()
        .name("noctalia-iced-palette".into())
        .spawn(move || {
            loop {
                ...
                if found != last {
                    ...
                    if let Some(palette) = found
                        && sender.unbounded_send(palette).is_err()
                    {
                        return;
                    }
                }
...
/// An iced [`Subscription`] that produces [`Palette`] changes as they happen.
pub fn subscription() -> Subscription<Palette> {
    Subscription::run(watch_stream)
}

fn watch_stream() -> impl Stream<Item = Palette> {
    let (_, changes) = watch();
    changes
}
